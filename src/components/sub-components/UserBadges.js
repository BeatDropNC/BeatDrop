  ...
  useEffect(() => {
    ...
    return (
      ...
      <button className={`badges-level-select-button ${levelToShow === 'level5' ? "selected-button" : ""}`} onClick={()=>{setLevelToShow('level5')}}>5</button>
        ...
        <img className={`badges-bronze badge ${badgesEarned.bronze ? 'badge-earned' : ""}`} src={bronze} alt='bronze badge'></img>
          <img className={`badges-silver badge ${badgesEarned.silver ? 'badge-earned' : ""}`} src={silver} alt='silver badge'></img>
          <img className={`badges-gold badge ${badgesEarned.gold ? 'badge-earned' : ""}`} src={gold} alt='gold badge'></img>
